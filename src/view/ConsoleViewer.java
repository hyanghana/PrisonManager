...
public class ConsoleViewer {
	...
	public void search() { // 검색메뉴
		...
		if(foundPrisoners == null || foundPrisoners.size() < 1) {
			...
			for(int i = 0; i < foundPrisoners.size(); i++) {
				...
				System.out.println("죄목 : " + foundPrisoners.get(i).getCrime()); // 나중에 수정
				System.out.println("구분 : " + foundPrisoners.get(i).getType());	// 나중에 수정
	...
	public void showAll() { // 전체출력
		...
		if(prisonMgr.ShowAll().size() < 1) {
			...
			System.out.println("죄목 : " + prisoners.get(i).getCrime());	// 나중에 수정
			System.out.println("구분 : " + prisoners.get(i).getType());	// 나중에 수정
